...
class AuthBypassScanner:
        ...
        if schema:
            findings.append(
                create_finding(
                    ...
                    evidence={"unauthenticated_introspection": "SUCCESS"},
            ...
            if result.get("data") == auth_result.get("data"):
                findings.append(
                    create_finding(
                        ...
                        cwe="CWE-306: Missing Authentication for Critical Function",
                        evidence={"unauthenticated_query": "SUCCESS"},
                        poc="{ __typename }",
                        url=self.client.url,
                    )
                )
        ...
        if sensitive_queries:
            findings.append(
                create_finding(
                    ...
                    evidence={"sensitive_queries": sensitive_queries},
        ...
        if sensitive_mutations:
            findings.append(
                create_finding(
                    ...
                    evidence={"sensitive_mutations": sensitive_mutations},
    ...
    def _test_field_level_auth(self) -> List[Dict]:
        ...
        if self.client.schema:
            ...
            user_type = None
            for t in types:
                if t.get("kind") == "OBJECT" and t.get("name", "").lower() in [
                    "user",
                    "admin",
                    "account",
                ]:
