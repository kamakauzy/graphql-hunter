            ...
            if result.get("_timeout"):
                findings.append(
                    create_finding(
                        ...
                        evidence={"depth": depth, "timeout": True},
                        poc=query[:300] + "...",
                    )
                )
            ...
            if result.get("data") and depth >= 10:
                findings.append(
                    create_finding(
                        ...
                        cwe="CWE-400: Uncontrolled Resource Consumption",
                        evidence={"depth_tested": depth, "accepted": True},
                        url=self.client.url,
                    )
                )
                ...
                if "complexity" in error_msg.lower():
                    findings.append(
                        create_finding(
                            ...
                            evidence={"complexity_check": "enabled"},
                            url=self.client.url,
                        )
                    )
                    return findings
        ...
        if result.get("data"):
            findings.append(
                create_finding(
                    ...
                    cwe="CWE-400: Uncontrolled Resource Consumption",
                    evidence={"complex_query_accepted": True},
                    url=self.client.url,
                )
            )
