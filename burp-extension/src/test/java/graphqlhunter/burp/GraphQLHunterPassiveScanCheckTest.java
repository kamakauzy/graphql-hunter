...
class GraphQLHunterPassiveScanCheckTest
{
    @Test
    void emitsIssuesForObservedGraphqlLeakage()
    {
        BurpIssuePublisher publisher = new BurpIssuePublisher(new FakeSiteMap(), null);
        GraphQLHunterPassiveScanCheck check = new GraphQLHunterPassiveScanCheck(testPublisher());
        HttpRequest request = request(
            ...
            List.of(
                ...
                header("Authorization", "Bearer eyJhbGciOiJIUzI1NiJ9.eyJleHAiOjEwLCJpYXQiOjEwfQ.signature")
            )
        );
        HttpResponse response = response(
            200,
            "{\"errors\":[{\"message\":\"Did you mean viewer?\\nTraceback (most recent call last)\"}],\"extensions\":{\"tracing\":true}}",
            List.of(
                header("Content-Type", "application/json"),
                header("X-GraphQL-Tracing", "enabled")
            )
        );

        assertFalse(check.issuesFor(requestResponse(request, response)).isEmpty());
    }
